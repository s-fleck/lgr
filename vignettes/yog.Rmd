...
This tells us that `lg` logs all events of at least level `info`. It does not 
have any Appenders on its own, but it dispatches all the LogEvents it creates 
to the Appenders of the root Logger (ignoring the threshold and filters of the 
root Logger, but not of the Appenders).

We can now use `lg$fatal()`, `lg$info()`, etc.. to log messages with this Logger:
```{r}
lg$info("A test message for lg")
```

If we do not want `lg` to dispatch to the root logger, we can simply switch
`propagate` to `FALSE`.
```{r}
...
Consequently, `lg` no longer outputs log messages
```{r}
lg$info("nothing will happen, since now there are no Appenders for lg")
```
