...
Filters are special functions that have exactly two arguments: 
  `event` (the LogEvent to be filtered) and
  `obj` (the Logger/Appender that calls `filter()`). 
They will be applied after the threshold is checked. 
...
lgr comes with simple but powerfull formatting syntax for LogEvents. Please
refere to `?format.LogEvent` for the full list of available placeholders.
...
lg <- Logger$new(
  ...
  appenders = AppenderBuffer$new(
    ...
    should_flush = function(event, obj) event$level <= 200,
