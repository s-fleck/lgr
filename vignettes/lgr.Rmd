...
name `foo/bar` whose parent is the logger `foo` whose parent is (implicitely)
the `root` logger. If the logger `foo` does not exist in that scenario, it is 
created automatically. This behaviour might sound strange at first, but it mimicks
tried and tested behaviour of python logging. This way logging ist decoupled
from the business logic and your program will not abort if you forgott to
